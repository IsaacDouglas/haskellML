...
type NaiveInstance = ([PairMV], Class) 
...
makeNaiveInstance d c = ([ (mean x, variance x) | x <- t ], c)
                where
...
proPosterior f (p, c) = ((aux f (p, c)), c)
                  where
                    aux [] _ = 1
                    aux (x:xs) (((m, v):ys), z) = (posteriorX x m v) * aux xs (ys, z)
